...
pub fn meta_f_a_element (set_element: &NameHashAddress) -> anyhow::Result<()> {
...
pub fn get_address_book_set () -> anyhow::Result<Vec<NameHashAddress>> {
    let database = open_db(HOT_DB_NAME)?;
...
fn shortcut_set_element (set_element: &NameHashAddress) -> anyhow::Result<MetaShortCut> {
    ...
    if shortcut.meta_values.name != set_element.name {
        return Err(Error::NameChanged(set_element.address.to_string()).show())
    }
    if shortcut.genesis_hash != set_element.genesis_hash {
        return Err(Error::GenesisHashChanged{address: set_element.address.to_string(), old_genesis_hash: set_element.genesis_hash, new_genesis_hash: shortcut.genesis_hash}.show())
    }
    Ok(shortcut)
}
